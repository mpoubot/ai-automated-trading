...
# Frozen S&P 100 security universe — corrected 2026-08-26.
#
# This is a SECURITY list, so Alphabet Class A and Class C are represented
# separately and the list contains 101 tickers for the 100-company index.
#
# The list is frozen for v0.4.8.1 and must not be altered based on observed
# trading results.
S_AND_P_100 = [
    "AAPL",
    "ABBV",
...
if len(S_AND_P_100) != EXPECTED_UNIVERSE_SIZE or len(set(S_AND_P_100)) != EXPECTED_UNIVERSE_SIZE:
    raise RuntimeError("AURA v0.4.8.1 requires exactly 101 unique frozen S&P 100 securities.")
...
def write_universe_once(outdir: Path, as_of_date: str):
    ...
    if not existing.empty:
        symbols = existing["symbol"].astype(str).str.strip().tolist()
        version_values = existing["universe_version"].astype(str).unique().tolist()
        if len(symbols) != EXPECTED_UNIVERSE_SIZE or len(set(symbols)) != EXPECTED_UNIVERSE_SIZE:
            raise RuntimeError(
                f"Existing {path} is not the corrected 101-security universe "
                f"(found {len(symbols)} rows / {len(set(symbols))} unique). "
                "Archive/delete AURA_LIVE and rerun the corrected version."
            )
        if version_values != [UNIVERSE_VERSION]:
            raise RuntimeError(
                f"Existing {path} belongs to a different universe version: {version_values}. "
                "Archive/delete AURA_LIVE before starting the clean v0.4.8.1 run."
            )
        return
    rows = [
        {
...
def main():
    ...
    print(f"Universe: {UNIVERSE_VERSION} ({len(S_AND_P_100)} securities / 100 companies)")
    ...
    print("S&P 100 universe is immutable once universe.csv exists (101 securities).")
