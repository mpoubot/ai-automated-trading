...
Purpose
-------
Test whether the CURRENT ENTRY SIGNAL becomes materially more profitable
when the EXIT architecture is changed, without changing the entry inventory.

This is deliberately an EXIT-ONLY experiment. It first runs the existing
...
Therefore:
  ...
  - same entry count and entry timestamps for every exit model

Exit models tested
------------------
CURRENT       : existing 3R partial -> breakeven -> ATR trail -> time stop
TP_1R         : full position at +1R, hard stop -1R
TP_1_5R       : full position at +1.5R, hard stop -1R
TP_2R         : full position at +2R, hard stop -1R
TP_3R         : full position at +3R, hard stop -1R
BE_1R_TRAIL   : no partial; when +1R is reached, stop moves to breakeven,
                then ATR trail is applied
PARTIAL_1_5R  : 50% at +1.5R, remaining 50% moves to breakeven and trails
...
Usage
-----
  python exit_sensitivity_test.py --days 720 --runsymbols
  python exit_sensitivity_test.py --days 720
...
def main():
    ...
    parser.add_argument("--select-by", default="expectancy",
                         choices=["expectancy", "profit_factor"],
                         help="PRE-DECLARED selection criterion — fixed before results are seen")
    parser.add_argument("--min-trades", type=int, default=100,
                         help="a model must produce at least this many research trades to be selectable")
    ...
    if not args.no_holdout:
        ...
        print(f"HOLDOUT period : {split_ts.date()} .. {all_end.date()}  "
              f"({args.days - research_days}d) — UNTOUCHED until one model is chosen")
    print(f"Selection rule : highest {args.select_by} on RESEARCH, "
          f"min {args.min_trades} trades. Chosen once, evaluated once.")
    print("The holdout must NOT be used to modify TP/SL/trailing/breakeven or anything else.")
    ...
    print("\n" + "=" * 78)
    print("RESEARCH RESULTS — all models (selection happens here, and only here)")
    print("=" * 78)
    ...
    # ---- select exactly one model, on research only ----------------------
    selected = None
    holdout_summary = pd.DataFrame()
    if not research_summary.empty:
        eligible = research_summary[research_summary["num_trades"] >= args.min_trades]
        if eligible.empty:
            print(f"\nNo model reached {args.min_trades} research trades — nothing selected.")
        else:
            selected = eligible.sort_values(args.select_by, ascending=False).iloc[0]["exit_model"]
            print(f"\nSELECTED MODEL (by {args.select_by} on RESEARCH only): {selected}")

    # ---- evaluate the selected model ONCE on the holdout ------------------
    if selected and not args.no_holdout and holdout_cache:
        models_for_holdout = ["CURRENT"] if selected == "CURRENT" else ["CURRENT", selected]
        holdout_windows = build_windows(holdout_cache, max(2, args.windows // 2))
        holdout_summary, holdout_trades = _run_models_on_period(
            models_for_holdout, holdout_cache, funding_cache, holdout_windows, "HOLDOUT")

        print("\n" + "=" * 78)
        print("HOLDOUT RESULT — the only number that is genuinely out-of-sample")
        print("=" * 78)
        ...
        print("\n" + "-" * 78)
        print(f"{'':<22}{'RESEARCH':>12}{'HOLDOUT':>12}{'DEGRADATION':>14}")
        for field in ["profit_factor", "expectancy"]:
            for model in dict.fromkeys(["CURRENT", selected]):
                r = _get(research_summary, model, field)
                h = _get(holdout_summary, model, field)
                print(f"{model+' '+field:<22}{r:>12.3f}{h:>12.3f}{h-r:>14.3f}")
        cur_h_pf = _get(holdout_summary, "CURRENT", "profit_factor")
        sel_h_pf = _get(holdout_summary, selected, "profit_factor")
        print("-" * 78)
        print(f"\nHoldout improvement of {selected} over CURRENT: "
              f"PF {sel_h_pf - cur_h_pf:+.3f}")
        print("""
How to read this:
  - Research numbers are selection, not evidence. The best of N models always
    looks good in-sample.
  - Only the HOLDOUT columns are out-of-sample. If the selected model's
    advantage over CURRENT shrinks to ~0 there, the research result was
    selection luck.
  - Large research->holdout degradation is the signature of overfitting.
  - Do NOT now pick a different model because it did better on the holdout.
    That would convert the holdout into a second research set and destroy
    the only clean evidence in this experiment.""")
    elif args.no_holdout:
        print("\nNo holdout was run. These numbers are in-sample only.")
    ...
    print("\nInterpretation rule:")
    print("  A better exit is only interesting if its advantage SURVIVES the holdout.")
    print("  Do not tune entry parameters from this run.")
