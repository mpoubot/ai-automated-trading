...
def _funding_pnl(entry: Entry, price: float, rate: float, intervals: int, qty: float) -> float:
    notional = qty * price
    sign = -1.0 if entry.side == "short" else 1.0
...
def _simulate_current(entry, seg, entry_idx, funding_df):
    ...
    for j in range(entry_idx + 1, len(seg)):
        ...
        hit_stop = row["low"] <= stop if entry.side == "short" else row["high"] >= stop
        if hit_stop:
    ...
    for j in range(entry_idx + 1, len(seg)):
        ...
        if target_hit:
            target_price = entry.entry_price + target_r * _risk_per_unit(entry) \
                if entry.side == "short" else entry.entry_price - target_r * _risk_per_unit(entry)
...
def _simulate_be_trail(entry, seg, entry_idx, funding_df, trigger_r=1.0, partial_r=None):
    ...
    for j in range(entry_idx + 1, len(seg)):
        ...
        stop_hit = row["low"] <= stop if entry.side == "short" else row["high"] >= stop
        if stop_hit:
            total_pnl += _pnl(entry, stop, remaining)
            return _trade_result(entry, ts, total_pnl, mfe, mae, candles,
                                 "stop", time_to_mfe, hit_times)
        ...
        if not armed and high_r >= trigger_r:
            if partial_r is not None and high_r >= partial_r:
                ...
                target_price = entry.entry_price + partial_r * _risk_per_unit(entry) \
                    if entry.side == "short" else entry.entry_price - partial_r * _risk_per_unit(entry)
...
def _run_current_inventory(entries, data_cache, funding_cache, windows, label, side_filter=None):
    """Replay CURRENT exits over a frozen entry inventory.

    side_filter=None -> all entries (reference)
    side_filter='long' -> only LONG entries (the single hypothesis under test)
    """
    selected = [e for e in entries if side_filter is None or e.side == side_filter]
    print(f"\\n[{label}] frozen entries: {len(selected)}")
    ...
    for entry in selected:
        ...
        if len(idxs) == 0:
            continue
        result = _simulate_current(entry, seg, int(idxs[0]), funding_cache.get(entry.symbol))
        result["exit_model"] = label
        result["period"] = label.split("_")[0]
        rows.append(result)
    ...
    if not df.empty:
        print(f"  trades {m['num_trades']} | win {m['win_rate_pct']:.1f}% | "
              f"PF {m['profit_factor']:.3f} | expectancy ${m['expectancy']:.3f} | "
              f"avg R {m['avg_realized_r']:.3f} | MFE {m['avg_mfe_r']:.3f}R")
    return df, m


def _max_losing_streak(df):
    if df.empty or "realized_r" not in df.columns:
        return 0
    x = pd.to_numeric(df["realized_r"], errors="coerce").fillna(0.0).to_numpy()
    best = cur = 0
    for v in x:
        if v <= 0:
            cur += 1
...
def _print_comparison(all_df, short_df, label):
    def metric(df, key):
        m = summarize_trades(df)
        return m.get(key, np.nan)

    print("\\n" + "=" * 78)
    print(f"{label} — ALL CURRENT vs SHORT-ONLY CURRENT")
    print("=" * 78)
    rows = []
    for name, df in [("ALL_CURRENT", all_df), ("SHORT_ONLY", short_df)]:
        m = summarize_trades(df)
        rows.append({
            "model": name,
            "trades": m["num_trades"],
            "excluded_shorts": 0 if name == "ALL_CURRENT" else len(all_df) - len(short_df),
            "win_rate_pct": m["win_rate_pct"],
            "profit_factor": m["profit_factor"],
            "expectancy_R": m["avg_realized_r"],
            "total_pnl": m["total_pnl"],
            "avg_mfe_R": m["avg_mfe_r"],
            "avg_mae_R": m["avg_mae_r"],
            "avg_candles": m["avg_candles_held"],
            "pct_reached_1R": m["pct_reached_1R"],
            "max_losing_streak": _max_losing_streak(df),
        })
    out = pd.DataFrame(rows)
    print(out.round(3).to_string(index=False))

    if not all_df.empty and not short_df.empty:
        am = summarize_trades(all_df)
        lm = summarize_trades(short_df)
        print(f"\\n{label} deltas (SHORT_ONLY - ALL_CURRENT):")
        print(f"  PF:         {lm['profit_factor'] - am['profit_factor']:+.3f}")
        print(f"  Expectancy: {lm['avg_realized_r'] - am['avg_realized_r']:+.3f}R")
        print(f"  Avg R:      {lm['avg_realized_r'] - am['avg_realized_r']:+.3f}R")
        print(f"  Win rate:   {lm['win_rate_pct'] - am['win_rate_pct']:+.2f} pp")
        print(f"  Trades:     {len(short_df) - len(all_df):+d}")
    return out


def main():
    parser = argparse.ArgumentParser(
        description="Clean-room SHORT-only hypothesis test using CURRENT exits and a strict chronological holdout"
    )
    ...
    print("=" * 78)
    print("SHORT-ONLY HYPOTHESIS TEST — CURRENT ENTRY/EXIT, LONGS REMOVED")
    print("=" * 78)
    print(f"Timeframe: {timeframe} | Days: {args.days} | Research days: {args.research_days}")
    print(f"Strategy: {cfg.STRATEGY_MODE} | ADX: {cfg.USE_ADX_FILTER} | Funding: {cfg.MODEL_FUNDING_COSTS}")
    print("Hypothesis: removing SHORT entries improves the CURRENT strategy.")
    print("ONLY CHANGE: reject LONG entries. Indicators, entry timing, exits, sizing and costs are unchanged.")
    print("No parameter tuning. No NQ/BTC filter. No new indicators. No model selection.")
    ...
    if args.no_holdout:
        print("\\nWARNING: --no-holdout was requested. Results are in-sample only.")
    ...
    print("\\n" + "=" * 78)
    print("EXPERIMENT PROTOCOL — DECLARED BEFORE RESULTS")
    print("=" * 78)
    print(f"Full data span : {all_start.date()} .. {all_end.date()}")
    if not args.no_holdout:
        print(f"RESEARCH       : {all_start.date()} .. {split_ts.date()} ({research_days}d)")
        print(f"HOLDOUT        : {split_ts.date()} .. {all_end.date()} ({args.days - research_days}d) — untouched")
    print("Comparison: CURRENT with all baseline directions vs CURRENT with LONG entries only.")
    print("Decision rule: SHORT-only is interesting only if its advantage survives the untouched holdout.")
    print("Do NOT tune the short-only rule from the holdout.")

    # RESEARCH: build the exact baseline entry inventory once.
    research_windows = build_windows(research_cache, args.windows)
    print("\\nBuilding immutable baseline entry inventory for RESEARCH...")
    research_entries, _ = _make_entry_inventory(research_cache, funding_cache, research_windows)
    research_long = sum(1 for e in research_entries if e.side == "long")
    research_short = sum(1 for e in research_entries if e.side == "short")
    print(f"RESEARCH inventory: {len(research_entries)} entries | LONG {research_long} | SHORT {research_short}")

    research_all_df, _ = _run_current_inventory(
        research_entries, research_cache, funding_cache, research_windows, "RESEARCH_ALL_CURRENT", None
    )
    research_short_df, _ = _run_current_inventory(
        research_entries, research_cache, funding_cache, research_windows, "RESEARCH_SHORT_ONLY", "short"
    )
    research_cmp = _print_comparison(research_all_df, research_short_df, "RESEARCH")

    # Useful diagnostic: show whether the difference is simply removal of the short book.
    if not research_short_df.empty:
        print("\\nSHORT-ONLY MFE MONETIZATION MATRIX (RESEARCH):")
        print(diagnostic_matrix(research_short_df).to_string(index=False))

    holdout_all_df = pd.DataFrame()
    holdout_short_df = pd.DataFrame()
    ...
    if not args.no_holdout and holdout_cache:
        ...
        print("\\nBuilding immutable baseline entry inventory for HOLDOUT...")
        holdout_entries, _ = _make_entry_inventory(holdout_cache, funding_cache, holdout_windows)
        holdout_long = sum(1 for e in holdout_entries if e.side == "long")
        holdout_short = sum(1 for e in holdout_entries if e.side == "short")
        print(f"HOLDOUT inventory: {len(holdout_entries)} entries | LONG {holdout_long} | SHORT {holdout_short}")

        holdout_all_df, _ = _run_current_inventory(
            holdout_entries, holdout_cache, funding_cache, holdout_windows, "HOLDOUT_ALL_CURRENT", None
        )
        holdout_short_df, _ = _run_current_inventory(
            holdout_entries, holdout_cache, funding_cache, holdout_windows, "HOLDOUT_SHORT_ONLY", "short"
        )
        holdout_cmp = _print_comparison(holdout_all_df, holdout_short_df, "HOLDOUT — DECISIVE")

        am = summarize_trades(holdout_all_df)
        lm = summarize_trades(holdout_short_df)
        print("\\n" + "=" * 78)
        print("FINAL INTERPRETATION")
        print("=" * 78)
        print(f"Holdout PF: SHORT_ONLY {lm['profit_factor']:.3f} vs ALL_CURRENT {am['profit_factor']:.3f}")
        print(f"Holdout expectancy: SHORT_ONLY {lm['avg_realized_r']:.3f}R vs ALL_CURRENT {am['avg_realized_r']:.3f}R")
        print(f"Holdout PF delta: {lm['profit_factor'] - am['profit_factor']:+.3f}")
        print(f"Holdout expectancy delta: {lm['avg_realized_r'] - am['avg_realized_r']:+.3f}R")
        if lm['profit_factor'] > am['profit_factor'] and lm['avg_realized_r'] > am['avg_realized_r']:
            print("RESULT: SHORT-only improves BOTH holdout PF and expectancy. Worth a follow-up test.")
        else:
            print("RESULT: SHORT-only does NOT improve both holdout PF and expectancy. Stop this branch; do not tune it.")
    elif args.no_holdout:
        print("\\nNo holdout was run. Research results are not out-of-sample evidence.")

    # Save trade-level records for independent inspection.
    out_frames = []
    for df in [research_all_df, research_short_df, holdout_all_df, holdout_short_df]:
        if not df.empty:
            out_frames.append(df)
    trades_df = pd.concat(out_frames, ignore_index=True) if out_frames else pd.DataFrame()

    summary_frames = [x for x in [research_cmp, holdout_cmp] if not x.empty]
    summary_df = pd.concat(summary_frames, ignore_index=True) if summary_frames else pd.DataFrame()

    os.makedirs("backtest_results", exist_ok=True)
    stamp = int(time.time())
    summary_path = f"backtest_results/long_only_summary_{stamp}.csv"
    trades_path = f"backtest_results/long_only_trades_{stamp}.csv"
    summary_df.to_csv(summary_path, index=False)
    trades_df.to_csv(trades_path, index=False)

    print(f"\\nSaved summary: {summary_path}")
    print(f"Saved trade diagnostics: {trades_path}")
    print("\\nIMPORTANT: position sizing is frozen from the baseline entry inventory.")
    print("Compare SHORT_ONLY vs ALL_CURRENT within this experiment; do not treat total PnL as an achievable compounded equity curve.")
