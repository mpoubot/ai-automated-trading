...
def load_trades(path: Path, min_entry_date: pd.Timestamp) -> pd.DataFrame:
    if not path.exists():
...
def main():
    ...
    trades = load_trades(Path(args.trades_csv), cutoff)
    ...
    print(f"Trades after cutoff:   {len(trades)}")
