...
def load_trades(
    ...
    # The live bot log does not currently contain a trade/position ID.
    # Therefore the same symbol+side can legitimately appear more than once
    # before an exit. We must NOT silently pretend that the mapping is exact.
    #
    # We keep a FIFO queue for each symbol+side. This gives us a deterministic
    # reconstruction while explicitly marking such cases as AMBIGUOUS. Exact
    # one-entry/one-exit trades remain "EXACT". Ambiguous trades are retained
    # for diagnostics but are excluded from the decisive validation metrics.
    open_positions = {}
    completed = []
    ambiguous_count = 0
    ...
    for _, row in events.iterrows():
        ...
        if is_entry:
            ...
            # More than one simultaneously open lot for the same symbol/side
            # cannot be mapped exactly without a position ID.
            if len(open_positions[key]) > 1:
                for pos in open_positions[key]:
                    pos["ambiguous"] = True
                ambiguous_count += 1
            continue

        if key not in open_positions or not open_positions[key]:
            # Exit without a matching entry in the supplied file.
            continue

        queue = open_positions[key]

        # FIFO: assign the next realized event to the oldest open lot.
        pos = queue[0]
                ...
                completed.append(
                    {
                        ...
                        "reconstruction": (
                            "AMBIGUOUS_FIFO" if pos["ambiguous"] else "EXACT"
                        ),
                    }
                )
    ...
    if "reconstruction" not in out.columns:
        out["reconstruction"] = "EXACT"
...
def main():
    ...
    valid_all = enriched.loc[matched].copy()

    exact = valid_all["reconstruction"].eq("EXACT")
    valid = valid_all.loc[exact].copy()
    ambiguous = valid_all.loc[~exact].copy()
    ...
    print(f"Exact trades:            {len(valid)}")
    print(f"Ambiguous FIFO trades:   {len(ambiguous)}")
    if len(ambiguous):
        print("WARNING: ambiguous trades are excluded from the decisive CCI30 metrics.")
        print("         The bot log has no trade/position ID, so their entry/exit")
        print("         relationship cannot be proven exactly.")

    if valid.empty:
        raise RuntimeError("No exactly reconstructed trades could be matched to CCI30.")
    ...
    print("- Ambiguous FIFO-reconstructed trades are excluded from the decisive metrics.")
