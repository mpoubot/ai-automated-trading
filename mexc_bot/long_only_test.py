...
def _print_comparison(all_df, long_df, label):
    def metric(df, key):
        m = summarize_trades(df)
        return m.get(key, np.nan)

    print("\\n" + "=" * 78)
    print(f"{label} — ALL CURRENT vs LONG-ONLY CURRENT")
    print("=" * 78)
    rows = []
    for name, df in [("ALL_CURRENT", all_df), ("LONG_ONLY", long_df)]:
        m = summarize_trades(df)
        rows.append({
            "model": name,
            "trades": m["num_trades"],
            "excluded_shorts": 0 if name == "ALL_CURRENT" else len(all_df) - len(long_df),
    ...
    if not all_df.empty and not long_df.empty:
        am = summarize_trades(all_df)
        lm = summarize_trades(long_df)
        print(f"\\n{label} deltas (LONG_ONLY - ALL_CURRENT):")
        ...
        print(f"  Trades:     {len(long_df) - len(all_df):+d}")
    return out


def main():
    parser = argparse.ArgumentParser(
        description="Clean-room LONG-only hypothesis test using CURRENT exits and a strict chronological holdout"
    )
    ...
    print("=" * 78)
    print("LONG-ONLY HYPOTHESIS TEST — CURRENT ENTRY/EXIT, SHORTS REMOVED")
    print("=" * 78)
    ...
    print("ONLY CHANGE: reject SHORT entries. Indicators, entry timing, exits, sizing and costs are unchanged.")
    ...
    print("Decision rule: LONG-only is interesting only if its advantage survives the untouched holdout.")
    print("Do NOT tune the long-only rule from the holdout.")
    ...
    research_long_df, _ = _run_current_inventory(
        research_entries, research_cache, funding_cache, research_windows, "RESEARCH_LONG_ONLY", "long"
    )
    research_cmp = _print_comparison(research_all_df, research_long_df, "RESEARCH")

    # Useful diagnostic: show whether the difference is simply removal of the short book.
    if not research_long_df.empty:
        print("\\nLONG-ONLY MFE MONETIZATION MATRIX (RESEARCH):")
        print(diagnostic_matrix(research_long_df).to_string(index=False))

    holdout_all_df = pd.DataFrame()
    holdout_long_df = pd.DataFrame()
    ...
    if not args.no_holdout and holdout_cache:
        ...
        holdout_long_df, _ = _run_current_inventory(
            holdout_entries, holdout_cache, funding_cache, holdout_windows, "HOLDOUT_LONG_ONLY", "long"
        )
        holdout_cmp = _print_comparison(holdout_all_df, holdout_long_df, "HOLDOUT — DECISIVE")

        am = summarize_trades(holdout_all_df)
        lm = summarize_trades(holdout_long_df)
        print("\\n" + "=" * 78)
        print("FINAL INTERPRETATION")
        print("=" * 78)
        print(f"Holdout PF: LONG_ONLY {lm['profit_factor']:.3f} vs ALL_CURRENT {am['profit_factor']:.3f}")
        print(f"Holdout expectancy: LONG_ONLY {lm['avg_realized_r']:.3f}R vs ALL_CURRENT {am['avg_realized_r']:.3f}R")
        ...
        if lm['profit_factor'] > am['profit_factor'] and lm['avg_realized_r'] > am['avg_realized_r']:
            print("RESULT: LONG-only improves BOTH holdout PF and expectancy. Worth a follow-up test.")
        else:
            print("RESULT: LONG-only does NOT improve both holdout PF and expectancy. Stop this branch; do not tune it.")
    ...
    out_frames = []
    for df in [research_all_df, research_long_df, holdout_all_df, holdout_long_df]:
        if not df.empty:
    ...
    print("Compare LONG_ONLY vs ALL_CURRENT within this experiment; do not treat total PnL as an achievable compounded equity curve.")
